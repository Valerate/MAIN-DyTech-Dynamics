...
function SciencePointGiver(NAME, AMOUNT)
	if not global.ResearchSystem then global.ResearchSystem = {} end
	if not global.ResearchSystem.Crafting then global.ResearchSystem.Crafting = {} end
	if not global.ResearchSystem.Crafting[NAME] then
		global.ResearchSystem.science = global.ResearchSystem.science + AMOUNT
		global.ResearchSystem.Crafting[NAME] = AMOUNT
	elseif global.ResearchSystem.Crafting[NAME] < 5 then
		global.ResearchSystem.science = global.ResearchSystem.science + AMOUNT
		global.ResearchSystem.Crafting[NAME] = global.ResearchSystem.Crafting[NAME] + AMOUNT
	else
		global.ResearchSystem.Crafting[NAME] = global.ResearchSystem.Crafting[NAME] + AMOUNT
	end
end 

function ScienceLabel()
	for _,player in pairs(game.players) do
		if player.gui.left["Science_Label"] then
			player.gui.left["Science_Label"].destroy()
		end	
		player.gui.left.add{type = "label", caption = "Science Points: "..global.ResearchSystem.science, name = "Science_Label"} 
	end
end
