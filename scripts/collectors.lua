...
for k,collector in pairs(global.Collectors.CollectorList) do
	if collector.valid then
		if not global.Collectors.AutomaticRange then
			items = game.surfaces[1].find_entities_filtered({area = {{x = collector.position.x - global.Collectors.Range, y = collector.position.y - global.Collectors.Range}, {x = collector.position.x + global.Collectors.Range, y = collector.position.y + global.Collectors.Range}}, name = "item-on-ground"})
			if #items > 0 then
				inventory = collector.get_inventory(1)
				for _,item in pairs(items) do
					if not item.to_be_looted then
			...
			items = game.surfaces[1].find_entities_filtered({area = {{x = collector.position.x - (AutoRange/global.Collectors.Amount), y = collector.position.y - (AutoRange/global.Collectors.Amount)}, {x = collector.position.x + (AutoRange/global.Collectors.Amount), y = collector.position.y + (AutoRange/global.Collectors.Amount)}}, name = "item-on-ground"})
			if #items > 0 then
				inventory = collector.get_inventory(1)
				for _,item in pairs(items) do
